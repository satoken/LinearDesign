        ...
        switch (beam_type) {
            ...
            case Beam_type::BEAM_P:
                ...
                for (auto& j_1_node_nucj_1 : dfa.left_edges[j_node]){
                    ...
                    if (hairpin_length == 5 or hairpin_length == 6 or hairpin_length == 8){
                        for(auto & seq_score_weight : hairpin_seq_score_cai[i_node][j_1_node][NTP(nuci, nucj_1)]){
                            ...
                            if (state.score == score_with_pair_penalty(pre_cal_score, i, j - 1) && abs(state.cai_score - pre_cal_cai_score) < epsilon){
                    ...
                    for (auto& i1_node_nuci : dfa.right_edges[i_node]){
                        ...
                        for (auto& j_2_node_nucj_2 : dfa.left_edges[j_1_node]){
                            ...
                            for (auto& i2_node_nuci1 : dfa.right_edges[i1_node]){
                                ...
                                if (state.score == score_with_pair_penalty(newscore, i, j - 1) && abs(state.cai_score - cai_score) < epsilon){
                    ...
                    if (state.score == score_with_pair_penalty(bestH[j_1_node][temp].score, i, j - 1)){ //no need to check CAI score here
                ...
                if (no_backpointer){
                    ...
                    if (state.score == score_with_pair_penalty(newscore, i, j - 1) && abs(state.cai_score - multi_state.cai_score) < epsilon){
                ...
                if (no_backpointer) {
                    for (auto& i1_node_nuci : dfa.right_edges[i_node]) {
                        if (get<1>(i1_node_nuci) != nuci) continue;
                        auto i1_node = get<0>(i1_node_nuci);
                        for (auto& j_1_node_nucj_1 : dfa.left_edges[j_node]) {
                            if (get<1>(j_1_node_nucj_1) != nucj_1) continue;
                            auto j_1_node = get<0>(j_1_node_nucj_1);
                            ...
                            auto temp_string = get_nuc_from_dfa_cai<IndexType>(dfa, i1_node, j_1_node, protein, best_path_in_one_codon_unit, aa_best_path_in_a_whole_codon, coding_start);
                            auto count = i + 1;
                            for (auto& nuc : temp_string) {
                                sequence[count] = nuc;
                                count++;
